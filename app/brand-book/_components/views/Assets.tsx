        ...
        if (e.target.files && activeBrandId) {
            ...
            for (const file of files) {
                try {
                    ...
                    let fileUrl = '';
                    try {
                        fileUrl = await uploadAsset(file, activeBrandId, activeProjectId || undefined);
                    } catch (err) {
                        console.warn("Supabase upload failed, falling back to local preview", err);
            ...
            <div className="w-64 bg-stone-50 border-r border-stone-200 flex flex-col h-full shrink-0">
                <div className="p-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <h3 className="text-[10px] font-bold text-stone-400 px-3 mt-8 mb-2 uppercase tracking-wider">Cloud Services</h3>
                        <div className="space-y-1">
                            <div className="flex items-center justify-between px-3 py-2 rounded-md text-sm text-stone-500 hover:bg-stone-100 cursor-pointer transition-colors group">
                                <div className="flex items-center gap-2">
                                    <span className="w-4 h-4 rounded bg-blue-100 text-blue-600 flex items-center justify-center text-[10px] font-bold">G</span>
                                    Google Drive
                                </div>
                                <span className="text-[10px] opacity-0 group-hover:opacity-100 bg-stone-200 px-1.5 py-0.5 rounded">Connect</span>
                            </div>
                            <div className="flex items-center justify-between px-3 py-2 rounded-md text-sm text-stone-500 hover:bg-stone-100 cursor-pointer transition-colors group">
                                <div className="flex items-center gap-2">
                                    <span className="w-4 h-4 rounded bg-orange-100 text-orange-600 flex items-center justify-center text-[10px] font-bold">C</span>
                                    Cloudflare
                                </div>
                                <span className="text-[10px] opacity-0 group-hover:opacity-100 bg-stone-200 px-1.5 py-0.5 rounded text-stone-700">Active</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="h-16 border-b border-stone-200 px-6 flex items-center justify-between bg-white shrink-0">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={() => fileInputRef.current?.click()}
                            disabled={uploading}
                            className="bg-stone-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-stone-800 flex items-center gap-2 transition-colors disabled:opacity-50"
                        >
